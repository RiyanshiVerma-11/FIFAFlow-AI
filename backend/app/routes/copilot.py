...
Provides the What-If scenario simulator, interactive natural-language
copilot chat, operational briefing generator, and post-match summary
report endpoints — all powered by the Gemini GenAI service with
...
def _get_stadium_state_summary(db: Session) -> Dict[str, Any]:
    """Aggregate live stadium telemetry into a context dictionary for AI prompts."""
    ...
    parking = next(
        (n.occupancy for n in nodes if "parking" in n.type.lower() or "Terminal" in n.name), 25
    )
...
async def what_if_simulation(
    ...
    """Run a hypothetical scenario simulation and return AI-generated impact analysis."""
...
async def copilot_chat(
    ...
    """Process an interactive natural-language query through the AI copilot."""
    state = _get_stadium_state_summary(db)
...
@router.get("/briefing")
async def get_latest_briefing(db: Session = Depends(get_db)) -> dict:
    """Retrieve the latest operational briefing or generate a fresh one via AI."""
    last_briefing = (
        db.query(AlertAndBriefing)
        .filter(AlertAndBriefing.type == "briefing")
        .order_by(AlertAndBriefing.created_at.desc())
        .first()
    )
    if last_briefing:
...
async def generate_report(
    ...
    """Generate a comprehensive AI-powered post-match operational summary report."""
    match = db.query(Match).first()
    nodes = db.query(DigitalTwinNode).all()
