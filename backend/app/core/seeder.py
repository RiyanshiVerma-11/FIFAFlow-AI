"""Database seed script for FIFAFlow AI mock presentation data.

Populates the database with initial users (organizer, volunteers, staff, fan),
a stadium record, a live match fixture, digital-twin nodes with simulated
occupancy, volunteer shift schedules, sustainability baseline logs, and an
initial operational briefing.
"""
...
def _random_fluctuation(low: int, high: int) -> int:
    """Return a random integer between *low* and *high* inclusive."""
    return random.randint(low, high)


def seed_database() -> None:
    """Initialise tables and populate base seed records for mock presentation mode."""
    Base.metadata.create_all(bind=engine)
    db: Session = SessionLocal()
    try:
        # Check if database already seeded
        if db.query(User).filter_by(username="organizer").first():
        ...
        vols = [
            Volunteer(user_id=volunteer1.id, shift_start=datetime.now(timezone.utc) - timedelta(hours=2), shift_end=datetime.now(timezone.utc) + timedelta(hours=4), status="active", assigned_zone="Zone A", location_json={"lat": 34.0522, "lng": -118.2437}),
            Volunteer(user_id=volunteer2.id, shift_start=datetime.now(timezone.utc) - timedelta(hours=1), shift_end=datetime.now(timezone.utc) + timedelta(hours=5), status="active", assigned_zone="Zone B", location_json={"lat": 34.0542, "lng": -118.2457}),
        ]
        db.add_all(vols)
        db.commit()
        ...
        for i in range(12):
            log = SustainabilityLog(
                ...
                energy_kwh=12000.0 + (i * 120) + _random_fluctuation(-100, 200),
                water_liters=40000.0 + (i * 350) + _random_fluctuation(-500, 1000),
                waste_kg=2000.0 + (i * 110) + _random_fluctuation(-100, 300),
        ...
        brief = AlertAndBriefing(
            type="briefing",
            content=(
                "FIFA World Cup 2026 Aztec Arena Command Center active. Attendance: 79,450. "
                "Gate 3 queue is at 18 minutes (high density). Volunteers deployed to perimeter links."
            ),
            severity="info",
        )
        db.add(brief)
